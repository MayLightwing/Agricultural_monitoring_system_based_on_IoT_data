...
| 时间序列状态平滑 | 使用 6 小时滑动窗口和 EMA 平滑维护连续状态信念 |
...
- 状态估计仍是轻量级概率规则与 EMA 平滑，尚未使用完整 Bayesian filter、HMM、Kalman filter 或 factor graph。
...
3. 将 EMA 平滑升级为 HMM、Bayesian filter 或 Kalman filter，实现更严格的时间状态估计。
